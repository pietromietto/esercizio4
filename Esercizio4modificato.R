...
ggplot(data=jap.data, aes(x = lret)) + geom_density(aes(y=..density..),col = "steelblue") + theme(axis.text.x = element_text(angle = 0, hjust = 1, size = 16), axis.text.y = element_text(size = 16))+ 
scale_color_manual(breaks = c("steelblue", "red"), values = c("blue" = "steelblue", "red" = "red"), labels = c("ν=8", "ν=ν.stima")) +
  theme(legend.text = element_text(color = "black", size = 14))+ labs( x = "rendimenti", y = "Frequenza") 

+ stat_function(fun=sstd,args=list(mean=fit.ged$par[1],sd=fit.ged$par[2],nu=fit.ged$par[3]),col=2,alpha=0.7,lwd=1)
#################
...
#adattiamo un modello che possa catturare l'eteroschedasticità presente nei dati


(fit1<-garchFit(~garch(10,0),data=res.m,cond.dist="norm",trace=F))
#i parametri sono tutti significativi, come ci si aspettava vista la forte persistenza dei quadrati
#Proviamo a rendere il modello più parsimonioso adattando un GARCH:
(fit2<-garchFit(~garch(2,1),data=res.m,cond.dist="norm",trace=F)) #alpha 2 non significativo
(fit3<-garchFit(~garch(1,1),data=res.m,cond.dist="norm",trace=F))
(fit4<-garchFit(~garch(1,1),data=res.m,cond.dist="ged",trace=F))
#stando alla significatività dei parametri il modello migliore sembra essere il GARCH(1,1)
N<-length(res.m)
(aic.1<-2*fit1@fit$llh + 2*12*N)
(aic.2<-2*fit2@fit$llh + 2*5*N)
(aic.3<-2*fit3@fit$llh + 2*4*N)
(aic.4<-2*fit4@fit$llh + 2*4*N)
